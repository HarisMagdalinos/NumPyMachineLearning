...
class ClassificationCNN(nn.Module):
    ...
    def __init__(self, input_dim=(3, 32, 32), num_filters=32, kernel_size=7,
        ...
        #out_size = ((width - kernel_size + 2*padding)/stride_conv) + 1
        view_size = int(num_filters*((int(width/2)*2)**2)/stride_pool**2)

        self.conv1 = nn.Conv2d(channels,num_filters,kernel_size=kernel_size,stride=stride_conv,padding=padding,bias=True)
        #self.conv1.weight.data.normal_(0,weight_scale)
        self.conv1.weight.data.mul_(weight_scale)
        
        self.conv2 = nn.Conv2d(num_filters,num_filters,kernel_size=kernel_size,stride=stride_conv,padding=padding,bias=True)
        #self.conv1.weight.data.normal_(0,weight_scale)
        self.conv2.weight.data.mul_(weight_scale)
        
        
        self.fc1 = nn.Linear(view_size,hidden_dim)
    ...
    def forward(self, x):
        ...
        x = F.max_pool2d(F.relu(self.conv2(x)),self.pool,stride=self.stride_pool)
